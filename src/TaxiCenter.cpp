...
void TaxiCenter::addNewTrip(Trip newTrip, ThreadPool *pool) {
    ...
    //if a trip isn't reachable (means it's vector path is empty=Null)- delete it from trips vector
...
int TaxiCenter::ifCabIdExists(int id) {
    int numOfCabs = this->cabs.size();
    for (int i = 0; i < numOfCabs; ++i) {
        if(cabs[i]->getID() == id)
            return -1;
    }
...
int TaxiCenter::ifTripIdExists(int id) {
    int numOfTrips = this->trips.size();
