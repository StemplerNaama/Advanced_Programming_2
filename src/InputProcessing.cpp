...
Grid* createGridAndObstacles() {
    ...
    //the input is numbers, now check their validation
...
void insertTrip(TaxiCenter* station, ThreadPool *pool) {
    ...
    //checks the validation of the trip
    ...
    isValid = station->ifTripIdExists(idOfTrip);
    if (isValid == -1) {
        cout << "-1" << endl;
        return;
    }
...
int tripInputProcessing(vector<string> &separatedMembers, Point* mapSize) {
    ...
    double tariff = atof(separatedMembers[6].c_str());
...
void insertCab(TaxiCenter* station) {
    string cabString;
    //getting the input of a cab from user
    cin >> cabString;
    ...
    isValid = station->ifCabIdExists(idOfCab);
    if (isValid == -1) {
        cout << "-1" << endl;
        return;
    }
...
void driverLocationRequest(TaxiCenter* station) {
    ...
    //gets input of driver from user
    cin >> idOfDriverStr;
...
void menu(TaxiCenter* station, Socket* tcp) {
    ...
    if (threadsFinish == numOfDrivers*count9extension) {
        cin >> extensionStr;
        if (!ifStringIsNum(extensionStr)) {
            cout << "-1" << endl;
            return;
        }
        extension = atoi(extensionStr.c_str());
        if (extension == 9)
            count9extension++;
        switch (extension) {
            case 1:
            ...
            case 9:
                ...
                break;
            case 7:
                //if we created thread pool(means- we inserted a trip)-terminate it+delete
                if(ifInitThreadPool == true) {
                    pool->terminate();
                    delete pool;
                }
                while (1) {
            ...
            default:
                //print -1 if the extension num isn't valid
                cout << "-1" << endl;
                break;
        }
    }
